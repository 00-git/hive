/**
 * Browser half: hive-fed-gateway's configuration seat on the Plugins page.
 *
 * alpha.2 renamed the browser seats. alpha.1's `settings.plugin.item` no longer
 * exists; the Plugins page (`dsh-client-ui-plugin-manager`) declares three and
 * states that a bundle's own configuration belongs in `plugins.bundle.config`
 * or `plugins.row.config`. A row seat is keyed `<package>#<rowId>` — the id the
 * bundle's patch declares — and the row's id only becomes an entry point once an
 * entry holds that key, so the seat *is* the row:
 *
 *   插件 → 已安装 → 查看 hive-fed-gateway → hive-fed-gateway/host → 表单
 *
 * Writes are staged. Every alpha.2 settings write is a revision-fenced document
 * mutation, so a draft becomes a document change exactly once — on 保存 — and the
 * Host stays the only authority on whether it landed. A card mounts only while
 * the Host serves its namespace, so a deployment without the owning half shows
 * no trace of it.
 *
...
/**
 * Render the gateway's configuration page: its controls and the save that writes
 * every staged edit.
